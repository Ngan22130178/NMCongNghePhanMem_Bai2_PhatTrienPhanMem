...
│   ├── main/
│   │   ├── java/
│   │   │   └── vn/edu/nlu/fit/nmcnpm/my_silly_bestie/
│   │   │       ├── controller/
│   │   │       │   ├── PetController.java
│   │   │       │   └── ToolController.java
│   │   │       ├── dao/
...
# 2. Hướng dẫn cài đặt môi trường phát triển (VS Code)

Dự án được phát triển trên trình soạn thảo **Visual Studio Code**, tận dụng hệ sinh thái Extension mạnh mẽ để tối ưu hóa hiệu suất lập trình Web.

* **Bước 1: Chuẩn bị phần mềm**
    * **VS Code:** Cài đặt bản mới nhất tại [code.visualstudio.com](https://code.visualstudio.com/).
    * **Trình duyệt:** Sử dụng Google Chrome hoặc Microsoft Edge để tận dụng bộ công cụ Inspect (F12) phục vụ việc căn chỉnh giao diện.

* **Bước 2: Thiết lập cấu trúc Project chuẩn**
    * Mở VS Code -> **File** -> **Open Folder** -> Chọn thư mục dự án.
    * Tổ chức thư mục theo tiêu chuẩn để quản lý tài nguyên hiệu quả:
        * `assets/images/`: Chứa hình ảnh thú cưng, icon dụng cụ.
        * `assets/audio/`: Chứa các file hiệu ứng âm thanh (.mp3).
        * `css/`: Lưu trữ file định dạng `style.css`.
        * `js/`: Lưu trữ file logic `script.js`.
    * **Lưu ý:** File `index.html` phải đặt ở thư mục gốc (root) để các công cụ tự động nhận diện.

* **Bước 3: Cài đặt các Extensions hỗ trợ**
    Vào mục Extensions (`Ctrl+Shift+X`) và cài đặt các công cụ sau:
    * **Live Server:** Tạo server ảo chạy game cục bộ, tự động cập nhật thay đổi trên trình duyệt ngay khi nhấn `Ctrl+S`.
    * **Prettier - Code formatter:** Tự động căn chỉnh mã nguồn HTML/CSS/JS theo chuẩn, giúp code sạch đẹp.
    * **Auto Rename Tag:** Tự động cập nhật thẻ đóng khi thay đổi thẻ mở trong HTML.

---

# 3. Hướng dẫn Deployment (Triển khai game)
...
1.  **Khởi tạo Git:** Mở Terminal (`Ctrl + \``) trong VS Code và thực thi:
    ...
    git init
    git add .
    git commit -m "Initial release: My Silly Bestie"
